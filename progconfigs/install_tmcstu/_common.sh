...
function sync_pkg() {
  ...
  mkdir -p "$pkgs_dir"
  cd "$pkgs_dir" || exit 0
  echo "==sync==: try syncing $name with:"
  echo "    $sync_cmd ${connection}:${src} ${dest}"
  if [[ -f "$dest" ]]; then
    echo "Warning: file $name already synced, will skip"
    return 0
  fi
  if ($sync_cmd "$connection:$src" "$dest"); then
    echo "Success: $name synced"
  else
    echo "Error: $name not synced"
  fi
  cd "$cwd" || exit 0
}
